...
| `user_settings` | `user_id`, `live_location_enabled` (default false), `push_enabled` | The per-user sharing toggle. |
| `families` | `color` | Drives pin colour for both check-ins and live markers. |
...
| `location.updated` | `{ user_id, family_id, lat, lng, accuracy_m, updated_at }` | `PUT /live-locations/me` commits | Everyone in the trip room **except the sender** |
| `location.cleared` | `{ user_id, trip_id, reason: "toggled_off"\|"stale"\|"stage_end" }` | Row deleted by any path | Everyone in the trip room |
...
- Live markers: family colour, visually distinct shape (a pulsing ring vs. the check-in teardrop),
  name label, freshness caption. The pulse respects `prefers-reduced-motion` and becomes a static
  ring when reduced motion is requested.
