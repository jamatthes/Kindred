...
- [x] Alembic migration (`0001_schema.py`, edited in place):
  - [x] `trip_category_settings`: unique index on `(trip_id, category)`; check constraints on
        `category` (`poll|region|accommodation|activity|meal`) and `voting_mode`
        (`score|thumbs`).
  - [x] New table `trip_stage_transitions` (`id`, `trip_id`, `from_stage`, `to_stage`,
        `direction`, `changed_by`, `created_at`), index on `(trip_id, created_at)`.
  - [x] `users`: add `last_login_at` (timestamptz null).
- [x] Backfill: for the existing trip, insert the five `trip_category_settings` rows with the
...
- [x] Extend the trip-creation seed so any future trip gets all five rows at creation.
- [x] Record `trip_stage_transitions`, `users.last_login_at` and the unique index as
...
- [x] `models/trip.py` — add `TripCategorySetting` and `TripStageTransition`.
- [x] Add `last_login_at` to the `User` model.
- [x] Update foundation's login route to set `users.last_login_at = now()` on success.
- [x] A `settings` accessor helper: `get_setting(key, default)` / `set_setting(key, value)`
...
- [x] `schemas/admin.py` — `TripAdminOut`, `TripPatchIn`, `StageChangeIn`,
...
- [x] `TripAdminOut` computes `can_advance_to`, `can_revert_to` and `blockers` server-side —
      the frontend must never derive transition legality itself.
- [x] Validation: `end_date >= start_date`; `timezone` is a valid IANA name (`zoneinfo`);
...
> NOTE (implementation, 2026-08-11): `POST /admin/trip/stage` below was superseded by the
> single-endpoint ruling recorded in `design.md` — stage transitions have exactly one route,
> `PATCH /api/v1/trips/{trip_id}/stage`, owned by `holiday-stage`. Everything this phase asks
> of that route (the legal pairs, the blockers, the conditional update, the history row) was
> implemented there, in `server/app/routers/trips.py`, with an ownership NOTE at the top of
> the file. `GET /admin/trip/stage-history` and the affordances stayed here.

- [x] `routers/admin.py`, every route with `Depends(require_organiser)` (owner OR organiser),
      except the Organisers routes added in Phase 6a, which use `Depends(require_owner)`.
- [x] `GET`/`PATCH /admin/trip`; the `PATCH` also carries
      `require_stage("planning", "holiday")`.
- [x] `POST /admin/trip/stage` — **no** `require_stage`, per the carve-out in
...
- [x] Apply the stage change as a **conditional update** (`WHERE stage = <expected from>`) so
      two concurrent admins cannot both succeed; zero rows affected →
      `409 illegal_transition`.
- [x] `GET /admin/trip/stage-history`.
...
- [x] `GET`/`PUT /admin/category-settings` (the `PUT` with
      `require_stage("planning", "holiday")`).
- [x] `GET /trip/category-settings` under `require_member` — the read every voting UI uses.
- [x] `existing_vote_count` per category: from `poll_scores` for `poll`, from
      `suggestion_votes` for the other four. Return zero when the table has no rows or does
      not yet exist at this milestone.
- [x] Self-healing read: if a category row is missing, insert it with its default before
...
- [x] `GET /admin/overview` with an optional `q` filter across display name, username and
      family name.
- [x] `POST /admin/users/{id}/reset-password` — generate a 4-word hyphenated password from a
...
- [x] `DELETE /admin/users/{id}` with `require_stage("planning", "holiday")` — delete the
      `family_members` row, revoke sessions, keep the `users` row and all authored content.
- [x] Guards: `cannot_target_self` on both routes; `last_family_head` on removal, reusing the
      same check `families` uses so the two cannot diverge.
- [x] Send `session.revoked` to the target's own socket, then close it.
...
- [x] `GET /admin/organisers` — `require_organiser`, lists `trip_organisers` joined to
      `users`/`family_members` for `OrganiserOut` per `design.md`.
- [x] `POST /admin/organisers` — `require_owner`, `{user_id}`, inserts a `trip_organisers` row
...
- [x] `DELETE /admin/organisers/{user_id}` — `require_owner`. Returns `409
...
- [x] Emit `organiser.appointed` / `organiser.demoted` to the whole trip room on each mutation
...
- [x] `GET`/`PATCH /admin/settings` — no stage guard. `invite_only: false` → `422
      not_implemented`.
- [x] Extend `services/google.py` with `probe() -> dict[str, ProbeResult]` covering Geocoding,
...
- [x] `GET /admin/google-status` — reads the stored `settings` blob, makes **no** network
      call.
- [x] `POST /admin/google-status/check` — rate-limited to 1/min via foundation's limiter,
      runs the probe, stores the blob with `checked_at` and `checked_by`, returns it.
- [x] Maps JS is reported as configured / not configured only, with no probe.
- [x] `GET /admin/stats` — one trip-scoped count per metric; hardcode zero for tables that do
...
- [x] Emit `stage.changed` to the whole trip room on every transition, forward or backward.
- [x] Emit `trip.updated` on name/date/timezone changes and
      `category_settings.updated` on voting-mode changes.
- [x] Emit `member.removed` on user removal, reusing the exact payload shape from `families`
      so one client handler serves both.
- [x] Emit `session.revoked` to the target user's own socket on reset and removal, then close
      it.
- [x] Emit `organiser.appointed` / `organiser.demoted` to the whole trip room (see Phase 6a) —
      **no** `session.revoked` on demotion.
- [x] Add `trip.updated`, `category_settings.updated`, `session.revoked`,
...
- [x] `web/src/features/admin/` with the section layout from `design.md`: single readable
      column, sticky section index on desktop, collapsed jump menu on mobile.
- [x] Nav rail / tab bar entry rendered for the owner and for organisers; a direct-URL access
      screen for everyone else.
- [x] **Trip** section — the four fields with all six states, explicit `Save` disabled until
      dirty, inline date validation, the "not decided yet" placeholder while in Planning.
- [x] `/setup/trip` route (AC-0) outside the app shell, rendered only when foundation's
...
- [x] `TripAdminOut.setup_complete` (`name` non-empty and `timezone` set) is computed
      server-side and is the same predicate foundation's gate reads — exported from one place,
      not reimplemented in the gate.
- [x] Seed the trip with `name = ''` and `timezone` from the container's `TZ` (falling back to
...
- [x] **Stage** section — current stage with its description, the forward primary action, the
      visually separated backward correction, disabled state with the blocker reason in words,
      and the stage-history table.
- [x] Confirm dialogs with the exact consequence copy from `design.md` and action-labelled
      buttons ("Start the holiday", "Freeze the trip") — never a bare "OK".
- [x] **Voting modes** section — five rows, segmented control, `Save`, and the
      existing-votes confirm naming the count.
- [x] **Families and members** section — both tables using the shared table pattern (tri-state
...
- [x] Row actions with real confirms; both actions disabled with an explanatory tooltip on the
      owner's own row.
- [x] Reset-password result as a copy-once block with a copy toast and the "shown only once"
      line.
- [x] **Instance** section — name field, registration radio group with the disabled options
      visible and annotated.
- [x] **Google APIs** section — the table, the `Run check` button with its plain caption and
      the one-minute countdown, status chips as icon plus word, inline hints on failures, and
      the never-checked empty state.
- [x] **Stats** section — a grid of labelled tabular numbers, zeroes shown, no chart.
- [x] **Organisers** section (AC-13) — rendered for the owner only (organisers see every other
...
- [x] Skeletons for each section while loading; spinners for inline saves.
...
- [x] Subscribe the app shell to `stage.changed`; update the cached trip and re-render.
- [x] Persistent archive banner in the End stage ("This trip has finished — everything is
      read-only"), not a toast.
- [x] A shared `useStage()` helper exposing `canMutate`, used by feature UIs to hide mutating
      controls rather than letting them fail on press. The backend guard remains the
      enforcement; this is presentation only.
- [x] Handle `session.revoked`: close the socket, clear local state, route to login with a
      plain message.
...
- [x] `test_admin_permissions.py` — every route in this feature returns `403` for a head/spouse
...
- [x] `test_admin_stage.py` — all legal and illegal transitions, blockers, the concurrency
      conditional update, history rows written with the correct direction, both by the owner
      and by an organiser.
- [x] `test_admin_stage_freeze.py` — with the trip in `end`, assert a representative mutating
...
- [x] `test_admin_users.py` — reset (session revocation, forced change, self-target refusal,
      plaintext returned once), removal (content retained, `last_family_head` refusal,
      End-stage refusal).
- [x] `test_admin_organisers.py` — appointment (idempotent re-appoint), demotion (family role
...
- [x] `test_category_settings.py` — member read allowed, member write refused, self-healing
      read, `existing_vote_count`.
- [x] `test_google_status.py` — all five classifications with the fake, the rate limit, and an
      assertion that `GET /admin/google-status` performs no call.
- [x] Vitest: the stage confirm dialogs render the right consequence copy, the disabled
