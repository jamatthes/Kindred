...
- **invites** — trip_id, family_id (nullable = invite creates a new family), token_hash (sha256; the raw token is shown once at creation and never stored), expires_at, created_by, used_by (nullable), used_at (nullable), revoked_at (nullable). Usable when `used_by is null and revoked_at is null and expires_at > now()`. `family_id` is `ON DELETE SET NULL`, so a deleted family leaves the invite reportable as `invite_family_missing` rather than vanishing with it. Indexed on family_id, trip_id and expires_at; token_hash unique. *families, `0002`*
