...
- **PROPOSED ADDITION (accepted, `distances`):** `distance.updated`, carrying `suggestion_id, family_id, status, duration_s, distance_m, is_estimate: false, computed_at`. Emitted **per row rather than per batch**, so a chip swaps from its straight-line estimate to the real driving value as soon as its own answer lands rather than waiting for the slowest sibling in the chunk. Rationale in `plan/features/distances/design.md` > WebSocket events.
...
- **distance_cache** — family_id, suggestion_id, duration_s, distance_m, mode (`driving`), computed_at; recomputed only when pin moves; unique (family_id, suggestion_id). **Implemented** at M3 with both PROPOSED ADDITIONS real: `status` (`pending`/`ok`/`no_route`/`failed`, check-constrained, default `pending`) and `attempts` (int, default 0). `status` is the column the table exists for — without it a pair with genuinely no driving route is indistinguishable from one nobody has computed, so every render would re-queue a paid API call for a pair that will never resolve; `no_route` is an *answer*, cached permanently and never automatically retried. `attempts` bounds the retry loop at `DISTANCE_MAX_ATTEMPTS` (3). Indexed on `(suggestion_id, status)` and `(family_id)`; both FKs cascade. **`no_home` is not a stored status** — it is derived at read time from a family having no coordinates. Nothing is ever expired by age: a distance between two fixed points does not change, and only a pin move past the 25 m epsilon or a home change invalidates a row. *distances*
...
- ~~**distance_cache.status (`ok`/`no_route`/`failed`), attempts** — unroutable pairs recorded once, never retried forever. *distances*~~ — **implemented**; specified in full under Agreed plan above. The stored set is `pending`/`ok`/`no_route`/`failed`, `pending` included, so a claimed-but-unanswered pair is distinguishable from an uncomputed one and a concurrent read can say "pending" rather than nothing.
...
| Distance Matrix | Once per (family home, suggestion) pair (server background task on suggestion create/move) | Forever in `distance_cache`. **Structurally enforced** since M3: `app/services/distance_matrix.py` is the only module that calls it, `app/services/distance_tasks.py` is its only importer and runs from background tasks alone, and `app/services/distances.py` — which every render path uses — imports neither. Two tests fail if that stops being true |
