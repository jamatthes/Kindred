...
The first deploy seeds a platform account `admin`/`admin` and **forces a password change on first login**, after which the owner names the trip and then their own family before reaching the app (revised 2026-08-11).

**How people get in.** The owner (or an organiser) invites each family with a new-family invite link. The recipient registers, then names their family on a setup screen and becomes its head of family. From there the head or a spouse invites their own family's members (or the owner/an organiser does it for them); neither a head nor a spouse can ever invite into another family. Every account is created through an invite — there is no open sign-up. **A family is only ever created by the person who becomes its head**, in the same transaction: there is no way for anyone, the owner included, to create a family they are not in, so there are no memberless family shells. The owner's own family is made this way too, on the same screen, during their onboarding.
...
| Onboarding | One server-owned gate: `auth/me.next_step` ∈ `change_password` / `setup_trip` / `setup_family` / `app`. The client routes on that field alone. The main admin names the trip on first login (`admin-console` AC-0); a new family's admin names their family on first login (`families` FM-13) — the family name is **not** collected on the join form (added 2026-08-11). **Revised 2026-08-11: the owner is not exempt.** No account reaches the app on its way in without a family, and that includes the owner, whose order is `change_password` → `setup_trip` → `setup_family` → `app`: they name **their own** family on the same screen everyone else uses, become its head, and their location sharing starts on per the head rule. Ownership admits them to the setup route in place of the invite nobody sent them. The gate previously carved the platform admin out — the reasoning was that a family setup screen "would lock them out of their own instance", which was true only while the setup route also refused them. The cost of the carve-out was real: the owner reached the app with no family and, with no other tool on offer, created a family from the admin UI without being placed in it. **Relatedly: no memberless families.** A family is only born with a head — via a new-family invite or the owner's own setup step — and the bare `POST /families` create is withdrawn; organisers bring a family onto the trip by inviting the adult who will run it |
...
| Family colour palette (2026-08-11) | Grown from 8 to **24** curated colours, slots 1-8 unchanged so existing families never shift. A family's founder picks its colour on the family-setup screen from the free slots on the trip; a head or spouse can change it later, same exclusivity rule, server-enforced (`409 color_taken` on a race), broadcast live over `family.updated` so pins/badges/cards recolor everywhere without a reload. **Overflow:** once all 24 slots are claimed, the 25th and later families get a free colour wheel (`<input type="color">`) instead of a slot — the server only accepts a custom hex when the palette is exhausted. Wheel-picked colours escape the palette's theme-tuning and distinguishability guarantees; accepted because it only occurs from the 25th family on, and the identity badge ring is never the sole carrier of family identity (a name always accompanies it). Data model: `families.color` (smallint 1-24) becomes nullable, paired with a new nullable `families.color_custom` (hex text) and a DB `CHECK` that exactly one of the two is set. All rendering goes through one helper, `familyColor(family)` (`web/src/design/familyColor.ts`), so no call site branches on slot-vs-custom itself. See `plan/features/families/design.md` > Family colour palette. |
